...
struct ScanLibraryTests {
    static func main() async throws {
        ...
        try Data("{\"status\":\"running\",\"stage\":\"export\"}".utf8)
            .write(to: current.appendingPathComponent("refusion-progress.json"))
        let fallback = try await library.preview(newEntry)
        precondition(fallback.points.count == 2 && fallback.note != nil)
        try Data("{\"status\":\"completed\"}".utf8)
            .write(to: current.appendingPathComponent("refusion-progress.json"))
        let completed = try await library.preview(newEntry)
        precondition(completed.note == nil)
        print("PASS: 未完成融合標示備援預覽，完成後不誤報")
