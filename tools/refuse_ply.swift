...
//
//    swiftc -O -o /tmp/refuse arkit-3dgs-scanner/Capture/{Models,CaptureConfig,BlurFilter,RefusionEngine,DepthSampleFilter,SurfaceTSDF}.swift \
...
@main struct RefusePLY {
    ...
    static func apply(_ kv: [String: Float], to c: inout CaptureConfig) throws {
        for (k, v) in kv {
            ...
            case "fusionNearRangeM":      c.fusionNearRangeM = v
            case "fusionFarExclusionM":   c.fusionFarExclusionM = v
            case "fusionFarVoxelScale":   c.fusionFarVoxelScale = v
            default:
                throw Fail("不認得的參數 '\(k)'。可用：blurWeightPower/blurWeightHalfPx/"
                           ...
                           + "fusionNearRangeM/fusionFarExclusionM/fusionFarVoxelScale")
            }
        }
    }
