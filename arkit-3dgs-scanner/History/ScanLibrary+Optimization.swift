    ...
    func optimizeTraining(_ entry: ScanEntry,
        ...
        let worker = Task.detached(priority: .userInitiated) {
            ...
            let refined = await OfflinePoseRefinement.run(records: annotated, directory: source, rounds: 6, surfaceRefinement:true,
                isCancelled: { Task.isCancelled }, progress: { progress($0 >= 0.92 ? L10n.text("檢查照片對齊…") : $0 >= 0.78 ? L10n.text("驗證局部表面對齊…") : $0 >= 0.546 ? L10n.text("搜尋並驗證重訪視角…") : L10n.text("逐張匹配與驗證相機位置…"), $0 * 0.45) })
