...
- **Setup.** The scan's photo, softened, sits behind a card that says what will happen. Below it are three quality cards (Quick preview, Standard with a *Recommended* badge, High quality), each with a plain description and, after a run on this phone, its measured time at the chosen resolution. A full-width **Training resolution** switcher (Low, Medium, High (original)) follows, with one line on its pixel size and cost. Then a memory check, the one reminder that matters (the rest of the app stays usable; train on power), an **Advanced settings** button (a sheet with pose refinement, PPISP and anti-aliasing, and the preset's technical details), and **Start training**.
