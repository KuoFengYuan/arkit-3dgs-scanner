...
The home screen is the scan history. A grid of covers shows each scan's date, image and point counts. Badges mark LiDAR scans and scans with a share archive. There are two columns on iPhone and larger covers on iPad. The primary **Start scanning** button floats at the bottom, and the grid fades out beneath it.
- The toolbar has the language menu (globe, current language) and the scanning guide (**?**).
- **Select** turns the grid into a multi-select view with **Select all** and **Delete selected**. Selecting everything and deleting it is the "delete all" case, and the confirmation says so.
- A long press on a cover offers **Delete**. Every deletion is still confirmed.
- With no scans, the page says there is no scan history yet and shows the three steps and tips. On devices without AR support, the start button is disabled with an explanation.
