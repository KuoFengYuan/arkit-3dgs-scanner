...
- **Setup.** The scan's photo, softened, sits behind a card that says what will happen. Below it are three quality cards (Quick preview, Standard with a *Recommended* badge, High quality), each with a plain description and, after a run on this phone, its measured time at the chosen resolution. An **Iterations** row follows: the count computed from the scan's photos with − / + (steps of 1,000), a line saying how it was computed, the estimated time at this phone's measured speed, and **Use automatic** once changed. A full-width **Training resolution** switcher (Low, Medium, High (original)) follows, with one line on its pixel size and cost. Then a memory check, the one reminder that matters (the rest of the app stays usable; train on power), an **Advanced settings** button (a sheet with pose refinement, PPISP and anti-aliasing, and the preset's technical details), and **Start training**.
- **Training.** The live model fills the screen, with a short gesture hint. The bottom card shows a progress ring, the current stage in words, and the remaining time; below them are one quiet line of iteration, Gaussians, loss, PSNR and elapsed time, and Pause / Save progress / Stop, with a full-width **Finish and save model** below them (a confirmation anchored to it). A caption says whether switching to another app pauses the run or lets it continue in the background. Notices appear at the top only for pauses (including while a capture is open), throttling or memory limits.
- **Away from the screen.** Training keeps running while the user browses the app. The home screen's **Scan history** card adds a status line, *Training 3DGS · n%* or *3DGS training paused · n%*, and the scan's training card opens the live view again.
- **Resume.** A ring with the saved percentage, **Resume where you left off**, **Restart** and **Delete progress**.
- **Model.** The saved model in the orbit viewer, a summary (iterations, Gaussians, time, PSNR), then **Enhance model** and **Share 3DGS model** cards. **Enhance model** turns the panel into the same setup, titled *Enhance 3DGS model*, with the saved model's size, the quality cards as *4,000 / 10,000 / 20,000 more iterations*, the resolution switcher, the memory check, **Cancel**, and **Start enhancing**.
