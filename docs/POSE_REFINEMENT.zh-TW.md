...
停止掃描後，手機會為匯出的 3DGS 資料集精修 ARKit 相機姿態。只有照片本身確認有改善時才會套用修正；驗證未通過就保留原姿態，這些抽樣檢查可降低退步風險，但不能保證絕對精度，也不能保證所有 3DGS 訓練器都產生更好的結果。
...
相機模式（關閉 LiDAR）沒有這條流程所需的深度，改跑純影像版本：以 Lucas-Kanade 追蹤特徵，對三角化的 track 執行相同的聯合 BA，是否套用由保留集決定，沒有照片對齊檢查。詳見[無深度的姿態精修](CAMERA_ONLY_ACCURACY.zh-TW.md#無深度的姿態精修)。

## 照片對齊檢查

保留的特徵 track 與求解器共用同一套匹配與 LiDAR 深度，因此看不到求解器自己引入的誤差。重跑改版前的流程時，BA 通過了自己的保留集（5.78 → 5.42 px），但相鄰影格的照片反而對得更差。所以 `PhotometricPoseValidator` 量測重疊影格之間的影像紋理是否一致。NCC 是照片對齊的替代指標，不等於完整 3DGS 訓練損失。
...
- **投影保留率**：先計入所有輸入姿態有效的紋理樣本，再檢查候選姿態。整體至少保留 95%，保留率低於 90% 或共同樣本少於 400 的配對不得超過 15%。邊界／遮擋改變可能影響少數配對，因此不會只因一組保留不足就拒絕全場景。整組投影消失仍視為負面觀測，不能從評分中消失。
...
- **帶 ARKit 運動先驗的聯合求解**：所有可用影格依拍攝順序一起求解。
  - 間隔 0.5 秒內的相鄰關鍵幀，以每一步 0.3 mm 與 0.01° 的先驗尺度限制對 ARKit 相對運動的改動；這是求解權重，不是硬性誤差上限，也不是實測精度。
...
下表保留第 5 版的桌機測量，早於局部試算與投影保留率檢查。

以下為第 5 版桌機重跑兩份 iPhone 17 Pro 掃描，使用當時 App 預設設定（含局部表面階段）。NCC 變化為相對掃描已存姿態的中位數；沒有地面真值。
...
以相同兩組掃描的既有 review 姿態為基準，最終特徵姿態檔與第 5 版逐位元組相同。特徵照片驗證分別保留 98.15%／98.31% 的基準有效樣本，通過加入保留率的檢查。兩組都只先試算 48 張，原本完整流程分別有 244／497 個有效中間影格；深度內部接受的 3／15 個修正，均未通過後續照片檢查，因此沒有套用。含試算與驗證的單次桌機姿態處理由 11.21 → 8.34 秒、20.28 → 13.83 秒；快取狀態會影響數字，不是手機耗時。局部試算同時仍最多保留三張影像／深度。完整階段的成功、失敗與取消，保留最終照片檢查與原始姿態回退。
