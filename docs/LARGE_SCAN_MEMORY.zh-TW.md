...
| 原始 RGB、深度、信心圖 | 持續寫入磁碟，一次處理一張幾何影格，另最多預讀一張深度解析度 RGB；沒有新增照片總張數限制 |
| 停止後保留的即時格 | 完成 GPU 幾何釋放及預覽存檔後，將 CPU 格按比例縮至最多 100,000 格 |
| 磚錨點與剩餘格 | 保留局部位置、顏色、權重與方向位元，續掃可重新補充細節 |
| 離線融合格 | 沿用 96 MiB 估計預算及可用記憶體降額；128 B／格是預估，不是實測 RSS。超過 `fusionNearRangeM`（3 m）的深度使用獨立的 4 cm 格，遠距雜訊不再迫使近距表面粗化 |
| 插入暫存 | 手機直接序列插入，不建立分片候選副本；每 1,024 點檢查容量及中斷，包括粗化內部；中止的可變格網直接丟棄，不匯出半成品 |
| 相鄰深度快取 | 同時限制 8 個項目及 2 MiB 深度／信心陣列與精確格子有效性遮罩；LRU 淘汰，低於 384 MiB 可用空間時清空 |
...
`refusion-progress.json` 第 7 版新增距離優先統計，見 [表面共識](LIDAR_SURFACE_CONSENSUS.zh-TW.md)。569 幀重跑中，grid 峰值由 785,576 格（已粗化到 4 cm）降為 2 cm 的 718,650 格。
...
另見[實驗性表面重建](SURFACE_RECONSTRUCTION.zh-TW.md)：新增的 32 MiB 常駐 TSDF 區塊預算、256 MiB 暫存背景資料預算、保持輸出一致的預讀與混合備援。
