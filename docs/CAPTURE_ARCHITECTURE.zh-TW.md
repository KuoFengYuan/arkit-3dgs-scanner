...
[English](CAPTURE_ARCHITECTURE.md) | **繁體中文**
...
| `CaptureController` | MainActor 上協調掃描、後處理、驗收、匯出；UI 不直接修改 phase。 |
...
7. 新掃描清除舊平面圖、統計與預覽資源。非同步平面圖／後處理回呼檢查工作識別碼，不能修改下一輪工作。
...
App 預設繁體中文，首頁可切英文並記住選擇；文件英文優先。AppLanguage / L10n 管理動態訊息、錯誤、UI 與無障礙，詳見 [語系慣例](LOCALIZATION.zh-TW.md)。

## 操作體驗

首頁底部浮著開始鍵，點「掃描紀錄」卡片才開啟歷史；存下第一筆掃描前以「掃描 → 檢查與補掃 → 匯出與分享」說明流程。掃描 HUD 顯示階段與具名快門，進階設定由單一「掃描設定」開啟（見[介面設計](INTERFACE_DESIGN.zh-TW.md)），權限拒絕提供系統設定入口。驗收時沒有可用影像不能匯出；零點掃描仍可匯出校準影像與空點集；追蹤已失效時不能把新座標的資料接到既有掃描。

掃描、後處理與匯出期間無法直接關閉畫面；先停止掃描或等待工作完成。離開尚未匯出的驗收畫面會說明資料保留位置，可從首頁歷史紀錄重新預覽、分享及刪除；離開後無法恢復原本的即時掃描工作階段。
...
- 姿態增量使用完整旋轉矩陣，避免一階近似累積尺度／剪切誤差。「精細掃描」預設開啟，有 LiDAR 時以 ARKit 運動先驗聯合求解所有影格的 BA；必須通過保留集與照片對齊檢查才套用，見[姿態精修](POSE_REFINEMENT.zh-TW.md)。這會增加停止後的處理時間。
...
列表直接讀取 `Documents/scans/scan_*`，因此舊版掃描也會出現。缺少預覽的舊資料會以有限影格與點數預算從深度重建；沒有深度時仍可翻閱照片。預覽最多顯示 120,000 點，並使用影像縮圖限制記憶體。歷史匯出先補齊 COLMAP 訓練資料，再重新打包 ZIP。

歷史列表可按「選取」逐筆勾選，或按「全選」，再執行「刪除所選」；全部選取後刪除即為全部刪除，確認視窗會標明。長按封面也可刪除。確認視窗會顯示筆數，取消不更動檔案，處理期間停用操作及關閉手勢。
...
- **關閉**：不請求 sceneDepth、smoothedSceneDepth、sceneReconstruction，也不啟動 RoomPlan 或儲存深度；保留 RGB、ARKit 姿態及稀疏特徵點雲。可選用固定 ARKit 姿態的純 RGB 多視角取樣重建，詳見 [相機模式](CAMERA_ONLY_ACCURACY.zh-TW.md)。
...
```sh
swiftc arkit-3dgs-scanner/Capture/Localization.swift arkit-3dgs-scanner/Capture/TrainingFrameSelector.swift -module-cache-path /tmp/fable-swift-cache \
...
| `tools/test_bundle_adjust.swift`（19 項） | 合成姿態校正、平滑漂移聯合求解、無效觀測及保留集拒絕退步結果 |
| `tools/test_photometric_validation.swift`（8 項） | 照片對齊檢查接受漂移修正、拒絕逐幀抖動、只評分移動影格、資料不足與取消 |
