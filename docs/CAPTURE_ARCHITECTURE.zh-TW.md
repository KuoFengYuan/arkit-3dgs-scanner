...
首頁即掃描紀錄，底部浮著開始鍵；沒有掃描時以「掃描 → 檢查與補掃 → 匯出與分享」說明流程。掃描 HUD 顯示階段與具名快門，進階設定由單一「掃描設定」開啟（見[介面設計](INTERFACE_DESIGN.zh-TW.md)），權限拒絕提供系統設定入口。驗收時沒有可用影像不能匯出；零點掃描仍可匯出校準影像與空點集；追蹤已失效時不能把新座標的資料接到既有掃描。
