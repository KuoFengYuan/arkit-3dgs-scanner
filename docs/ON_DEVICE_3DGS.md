...
- **Quality:** three choices with plain descriptions. Once a run has finished on this phone, each choice also shows how long it took there. Nothing is extrapolated from another device.

  | Choice | Iterations | Training image long edge | Gaussian cap | SH degree |
  | --- | --- | --- | --- | --- |
  | Quick preview | 3,000 | 720 px | 300,000 | 2 |
  | Standard (recommended) | 7,000 | 960 px | 600,000 | 3 |
  | High quality | 15,000 | 1,280 px | 1,000,000 | 3 |
...
- **Automatic pauses:** the run saves a checkpoint and pauses when the app leaves the foreground (iOS allows no background GPU work), when the device is critically hot (it resumes once cooled to *fair*), below 15% battery unplugged, or under memory pressure. A *serious* thermal state or Low Power Mode slows the loop instead of pausing it.
...
- **Fitting the plan:** if the preset does not fit, the Gaussian cap is lowered. Densification never exceeds the cap.
...
- **`tools/test_gaussian_raster.swift`:** forward against a double-precision reference, with parameter and pose gradients checked by finite differences. It covers the Mip filter on and off, with and without capture motion (12 checks).
- **`tools/test_gaussian_loss.swift`:** loss, image and PPISP gradients.
- **`tools/test_gaussian_training.swift`:** 41 end-to-end checks.
  - Memory-plan fitting and overflow checks.
...
- **Simulator (iPhone 16 Pro):** covered the user flow only.
...
- One scan trains at a time. Training needs the app in the foreground.
