...
4. 沿著空間移動，從不同角度拍到同一表面。大空間裡，App 提示時（在新區域約 40 秒、4 m 後）請回到已拍過的區域重疊一段，回到時會提示已回到，讓漂移得到修正（見[重訪引導](docs/LOOP_CLOSURE_AND_SCALE.zh-TW.md#拍攝中的重訪引導)）。停止後等待資料優化，檢查點雲與缺漏，必要時「續掃」。
...
3. 選擇品質：
   ...
   | 品質 | 迭代次數 | 高斯上限 | 適合 |
   | --- | --- | --- | --- |
   | 快速預覽 | 4,000 | 300,000 | 先快速看看效果 |
   | 標準（建議） | 10,000 | 600,000 | 大多數掃描 |
   | 高品質 | 20,000 | 1,000,000 | 細節最多；時間最長、較耗電 |

   訓練照片超過約 333 張時會增加迭代次數，讓每張照片仍約訓練 12／30／60 次。卡片下方的「迭代次數」一列會顯示次數，以及依這支手機實測速度的預估時間；開始前可用 − / + 調整。
...
模型以 LichtFeld Studio 的密化策略 MRNF 為基底，再加上本專案自己的方法。訓練器會微調相機姿態、用 LiDAR 深度補上沒有點的表面、在訓練中補洞，並以 LiDAR 深度作為幾何損失，讓模型在偏離拍攝路徑環繞檢視時依然維持正確形狀。更快的反向傳播讓迭代次數提高到約 1.4 倍：三份重播掃描的保留影像 PSNR 提高 0.7–0.9 dB，標準品質在 Mac 上的時間也少了 10%。方法、實測結果、記憶體保護與檔案格式，詳見[手機端 3DGS 訓練](docs/ON_DEVICE_3DGS.zh-TW.md)；程式如何組織，見[訓練架構](docs/ON_DEVICE_3DGS_ARCHITECTURE.zh-TW.md)。iPhone 上的訓練速度、記憶體用量與發熱都還沒實測，Mac 與 Simulator 的結果不能代替。
