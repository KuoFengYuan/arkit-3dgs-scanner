...
- **實驗性表面重建：** 有上限的局部 RGB-D 姿態精修與稀疏 TSDF 表面點，保留完整 voxel 融合備援。融合另加入單張 JPEG 預讀與精確深度有效性快取，不改取樣或門檻。詳見[流程、效能比較與限制](docs/SURFACE_RECONSTRUCTION.zh-TW.md)。
...
3 m 以外的深度現在只補近距視角沒量到的表面，並使用 4 cm 格。實機掃描中遠距 LiDAR 有 1–5 cm 隨距離變化的偏移，會形成第二層表面。桌機重跑同一份 569 幀房間掃描，以目前融合結果的區域中心量測，局部厚度中位數由 **7.71 cm 降到 3.67 cm**，grid 也不再粗化到 4 cm；近距掃描沒有變化。只從遠處看到的表面仍保有遠距誤差。詳見 [距離優先](docs/LIDAR_SURFACE_CONSENSUS.zh-TW.md)。
