...
- **檢視與歷史：** 浮動面板放數據、主要的匯出或分享動作與次要動作。品質資訊放在獨立面板；歷史的「更多操作」包含空間尺度驗證、資料優化與刪除。
...
相機模式依賴紋理、清晰度與視差，不能把影像推估深度視為感測器量測。相機模式停止掃描後，以 PatchMatch 多視角立體為最多 48 個視角估計深度圖，只保留相鄰視角確認過的深度。以 App 自己的相機模式姿態重播兩份掃描，得到 11,000–14,500 點，是先前影像重建的 15–53 倍；5 cm 內涵蓋 LiDAR 表面的 10–24%（先前不到 8%）。詳見 [相機模式精度](docs/CAMERA_ONLY_ACCURACY.zh-TW.md)。
