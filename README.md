...
6. **無空白換圖**：下一張解碼期間保留上一張照片，完成後直接替換，不在每次切換時閃出載入畫面。點雲標記及拍攝時間跟隨實際已呈現的照片；高 FPS 遇到解碼較慢時等待該張載入結果，再前進下一張，避免連續取消請求造成空白或停滯。
...
上述回歸測試在本次開發過程中通過。原有單筆歷史 UI 已用合成資料驗證列表、點雲、照片切換、刪除確認與打包；後續多選、雙畫面回放、FPS 調整、自動跟隨及換圖閃爍修正的實際點擊／排版驗證，因 Simulator UI 自動操作連線逾時尚未完成。LiDAR 開關效果、實際精度、長掃描記憶體與溫度仍須真機測試。
